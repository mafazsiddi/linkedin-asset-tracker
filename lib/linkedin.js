...
// ---- Company engagement (which companies actually saw an ad set) ----

// LinkedIn's demographic pivots answer "who saw this", and MEMBER_COMPANY is the one Campaign
// Manager's Companies report is built on. Unlike CAMPAIGN/CREATIVE, demographic pivots are only
// served at timeGranularity=ALL — asking for DAILY returns a single undated aggregate row rather
// than one row per day, so there is no way to store this per-day and re-slice it later. Everything
// here is therefore keyed by the exact (campaign, start, end) window it was fetched for.
const COMPANY_ANALYTICS_FIELDS =
  'pivotValues,impressions,clicks,costInLocalCurrency,approximateMemberReach,oneClickLeads';
...
function mockCompanyEngagement(campaign, startISO, endISO) {
  const base = seedFrom(String(campaign.li_campaign_id || campaign.id) + startISO + endISO);
  return MOCK_COMPANIES.map((name, i) => {
    ...
    return {
      companyUrn: `urn:li:organization:${1000 + i}`,
      companyId: String(1000 + i),
      name,
      impressions,
      clicks,
...
// Turns organization URNs into names. There is no organization-name field on the analytics
// response — the pivot value is a bare URN — and the plain /rest/organizations lookup needs an
// organization-admin scope this app doesn't have. adTargetingEntities is the lookup that the ads
// scopes do cover, which is why it's used here rather than the more obvious endpoint.
//
// Deliberately best-effort: a company whose name can't be resolved still shows up, labelled by its
// URN, because knowing *how many* companies engaged is useful even when one name is missing.
async function resolveOrganizationNames(urns) {
  const out = {};
  ...
  for (let i = 0; i < unique.length; i += BATCH) {
    ...
    try {
      ...
      for (const el of data.elements || []) {
        ...
        if (urn && el.name) out[urn] = el.name;
      }
    } catch (e) {
...
async function fetchLiveCompanyEngagement(campaign, startISO, endISO) {
  const campaignUrn = encodeURIComponent(`urn:li:sponsoredCampaign:${campaign.li_campaign_id}`);
  const qs =
    `q=analytics&pivot=MEMBER_COMPANY&timeGranularity=ALL` +
    `&dateRange=${dateRangeParam(startISO, endISO)}` +
    `&campaigns=List(${campaignUrn})&fields=${COMPANY_ANALYTICS_FIELDS}&count=${PAGE_SIZE}`;
  const data = await linkedInGet('adAnalytics', qs);
  ...
  const names = await resolveOrganizationNames(rows.map(r => r.companyUrn));
  for (const r of rows) r.name = names[r.companyUrn] || null;
...
async function getCampaignCompanyEngagement(campaign, startISO, endISO) {
  ...
  const mode = process.env.LINKEDIN_MODE || 'mock';
  if (mode === 'live') return fetchLiveCompanyEngagement(campaign, startISO, endISO);
  return mockCompanyEngagement(campaign, startISO, endISO);
}
