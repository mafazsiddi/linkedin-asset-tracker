...
async function resolveViaTargetingEntities(urns, out) {
  ...
  for (let i = 0; i < urns.length; i += BATCH) {
    const batch = urns.slice(i, i + BATCH);
    try {
      const data = await linkedInGet(
        'adTargetingEntities',
...
//
// Sequential rather than parallel on purpose: adAnalytics throttles on metric values returned in a
// 5-minute window, and a title pivot can return 100 rows × 6 metrics on its own.
async function getCampaignAudienceBreakdown(campaign, startISO, endISO) {
  ...
  const jobFunctions = await fetchLiveDemographicPivot(
    campaign, MEMBER_JOB_FUNCTION_PIVOT, startISO, endISO
  );
  const jobTitles = await fetchLiveDemographicPivot(
    campaign, MEMBER_JOB_TITLE_PIVOT, startISO, endISO
  );
