...
// Lists the ads (creatives) actually attached to a campaign in LinkedIn. Assets only ever get
// per-ad numbers once their li_creative_id is filled in, and nothing in the app populated it —
// this is what lets the UI offer the real IDs to pick from instead of asking someone to hunt
...
async function listCampaignCreatives(campaign) {
  ...
  const mode = process.env.LINKEDIN_MODE || 'mock';
  if (mode !== 'live') {
    // Mock mode has no real creatives to enumerate; deterministic stand-ins keep the UI testable.
    return [0, 1, 2].map(i => ({
      creativeId: `${campaign.li_campaign_id}00${i}`,
      name: `${campaign.name} — creative ${i + 1}`,
      status: 'ACTIVE'
    }));
  }
  const campaignUrn = encodeURIComponent(`urn:li:sponsoredCampaign:${campaign.li_campaign_id}`);
  const qs = `q=criteria&campaigns=List(${campaignUrn})&count=${PAGE_SIZE}`;
  const data = await linkedInGet('creatives', qs);
  return (data.elements || []).map(el => {
    const id = String(el.id || '').split(':').pop();
    return {
      creativeId: id,
      // Creatives have no first-class name; the intended-status/reference is the closest label.
      name: (el.content && el.content.reference) || el.intendedStatus || id,
